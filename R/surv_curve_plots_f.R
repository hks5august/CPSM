...
surv_curve_plots_f <- function(Surv_curve_data, selected_sample, font_size,  
  ...
  Surv_curv_plot_highlight <- ggplot(survCurves_m, aes(x = Time, y = Value)) +
    geom_line(data = subset(survCurves_m, Patient != selected_sample),
              aes(group = Patient),
              color = all_line_col, size = line_size) +
    geom_line(data = subset(survCurves_m, Patient == selected_sample),
              aes(group = Patient, color = "Selected"),
              size = line_size) +
    scale_color_manual(values = c("Selected" = highlight_col)) +
    labs(x = "Time in Months", y = "Survival Probability", color = "Highlighted") +
