...
Nomogram_generate_f <-
  function(data, Feature_List, surv_time, surv_event) {
    # set.seed(7)

    # Check if any input variable is empty
    if (length(data) == 0 ||
      length(Feature_List) == 0 || length(surv_time)
    ...
    sel_data <-
      as.data.frame(data[, colnames(data) %in% c(ftr_list$ID), ])
    ...
    surv_1 <-
      function(x) {
        surv(1 * 12, lp = x)
      } # defined time.inc,1 year OS
    surv_2 <-
      function(x) {
        surv(1 * 36, lp = x)
      } # defined time.inc,3 year OS
    surv_3 <-
      function(x) {
        surv(1 * 60, lp = x)
      } # defined time.inc,5 year OS
    surv_4 <-
      function(x) {
        surv(1 * 120, lp = x)
      } # defined time.inc,10 year OS
    ...
    C_index_mat <-
      cbind(bias_corrected_c_index, bias_corrected_c_index)
