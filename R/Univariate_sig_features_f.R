...
Univariate_sig_features_f <-
  function(train_data,
    ...
    for (i in seq(
      ...
      # Create survival object
      surv_object <-
        Surv(time = tr_data1$OS_month, event = tr_data1$OS)

      # Survival analysis: fits cox ph model to find HR for median cut
      fit1 <-
        survfit(surv_object ~ (tr_data1[, i]) > (median(tr_data1[1, i])),
          data = tr_data1
        )
      # summary(fit1)
    ...
    sel_univ_train <-
      tr_data1[, colnames(tr_data1) %in% selected_feature_names,
        drop = FALSE
      ]
    ...
    sel_univ_test <-
      te_data1[, colnames(te_data1) %in% selected_feature_names,
        drop = FALSE
      ]
    # Convert to data frame if necessary
    ...
    tr_data2 <-
      tr_data_clin[, !colnames(tr_data_clin) %in% c("OS", "OS_month")]
    te_data2 <-
      te_data_clin[, !colnames(te_data_clin) %in% c("OS", "OS_month")]
      ...
      tryCatch(
        {
          # Survival analysis: fits cox ph model to find HR for median cut
          fit2 <-
            survfit(surv_object ~ tr_data2[, i], data = tr_data2)
          # summary(fit1)
          fit2.coxph <-
            coxph(surv_object ~ tr_data2[, i], data = tr_data2)
          # summary(fit1.coxph)
          first2 <- coef(summary(fit2.coxph))

          # Check whether the p-value is significant (< 0.05) or not
          if ((first2[5] <= 0.05) &&
            (!is.na(first2[5])) && (!is.na(first2[2]))) {
            # Store results in the list
    ...
    # Prepare test data with selected features
    sel_univ_test2 <-
      te_data2[, colnames(te_data2) %in% selected_feature_names2,
        drop = FALSE
      ]
    # Convert to data frame if necessary
