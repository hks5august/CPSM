...
Lasso_PI_scores_f <-
  function(train_data,
    ...
    surv_object <-
      survival::Surv(time = tr_data1$OS_month, event = tr_data1$OS)

    # develop model to select key features using LASSO method with user-defined folds
    ...
    sel_train2 <-
      as.data.frame(tr_data1[, colnames(tr_data1) %in% c(row.names(sel_features2)), ])
    # Make final files with the selected features (genes here)  and combine survival information
    train_feature_mat2 <-
      cbind(tr_data1["OS_month"], tr_data1["OS"], sel_train2)
    ######################## PI Index ########################
    ...
    test_feature_mat2 <-
      cbind(te_data1["OS_month"], te_data1["OS"], sel_test)
