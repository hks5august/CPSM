...
MTLR_pred_model_f <-
  function(train_clin_data,
    ...
    if (length(train_clin_data) == 0 || length(test_clin_data) == 0 ||
      ...
      length(test_features_data) == 0 ||
      length(Clin_Feature_List) == 0 ||
    ...
    if (Model_type == 1) {
      ...
      sel_clin_tr1 <-
        cbind(tr_clin1["OS"], tr_clin1["OS_month"], sel_clin_tr)
      sel_clin_te1 <-
        cbind(te_clin1["OS"], te_clin1["OS_month"], sel_clin_te)
      ...
      survCurves1 <-
        predict(Mod1, sel_clin_te1, type = "survivalcurve")

      # define column names
      colnames(survCurves1) <- c("time_point", rownames(sel_clin_te2))
      survCurves1_df <- as.data.frame(survCurves1)
      survivalcurve1 <-
        predict(Mod1, sel_clin_te2, type = "survivalcurve")
      Survival_prob_event1 <-
        predict(Mod1, sel_clin_te2, type = "prob_event")
      ...
      # Predicted Median
      medianSurv1_tr <-
        predict(Mod1, sel_clin_tr2, type = "median_time")
      ...
      survivalProbs_p1_tr <-
        predict(Mod1, sel_clin_tr2, type = "prob_times")
      ...
      # Test data
      survivalProbs_p1 <-
        predict(Mod1, sel_clin_te2, type = "prob_times")
      # extract prob times at diff times points
      ...
      surv_obj1_tr <-
        survival::Surv(sel_clin_tr2$OS_month, sel_clin_tr2$OS)
      ...
      c_index1_tr <- round(
        SurvMetrics::Cindex(surv_obj1_tr,
          predicted =
            medianSurv1_tr
        ),
        2
      ...
      # create survival object
      surv_obj1 <-
        survival::Surv(sel_clin_te2$OS_month, sel_clin_te2$OS)
      ...
      # Calculate Concordance Index
      c_index1 <- round(
        SurvMetrics::Cindex(surv_obj1, predicted = medianSurv1),
        2
      )
    ...
    ########################## Model with only PI score #######################
      ...
      # prediction on Test data
      survCurves2 <-
        predict(Mod2, sel_clin_te2, type = "survivalcurve")
      ...
      Survival_prob_event2 <-
        predict(Mod2, sel_clin_te2, type = "prob_event")
      ...
      # Predicted Median
      medianSurv2_tr <-
        predict(Mod2, sel_clin_tr2, type = "median_time")
      ...
      # time
      mean_median_surv2_d <-
        cbind(names_2, meanSurv2_d, medianSurv2_d)
      ...
      survivalProbs_p2_tr <-
        predict(Mod2, sel_clin_tr2, type = "prob_times")
      # extract prob times at diff times points
      ...
      survivalProbs_t_mat1_t2_2_tr <-
        survivalProbs_t_mat1_t_2_tr[, -1]


      # test data prediction
      survivalProbs_p2 <-
        predict(Mod2, sel_clin_te2, type = "prob_times")
      # extract prob times at diff times points
      ...
      # Concordance Index
      c_index_2_tr <- round(
        SurvMetrics::Cindex(surv_obj_2_tr,
          predicted = medianSurv2_tr
        ),
        2
      )
      ...
      # survival curve data
      survCurves3 <-
        predict(Mod3, sel_clin_te2, type = "survivalcurve")
      # survCurves is pretty large so we will look at the first 5 rows/columns.
      ...
      # training
      medianSurv3_tr <-
        predict(Mod3, sel_clin_tr2, type = "median_time")
      # test
      ...
      mean_median_surv3_d <-
        cbind(names_3, meanSurv3_d, medianSurv3_d)
      ...
      survivalProbs_p3_tr <-
        predict(Mod3, sel_clin_tr2, type = "prob_times")

      # test
      Survival_Probs_event3 <-
        predict(Mod3, sel_clin_te2, type = "prob_event")

      survivalProbs_p3 <-
        predict(Mod3, sel_clin_te2, type = "prob_times")
      ...
      # Concordance Index
      c_index_3_tr <- round(
        SurvMetrics::Cindex(surv_obj_3_tr,
      ...
      survCurves5 <-
        predict(Mod5, sel_clin_te2, type = "survivalcurve")
      # add column names
      ...
      # Median
      medianSurv5_tr <-
        predict(Mod5, sel_clin_tr2, type = "median_time")
      ...
      mean_median_surv5_d <-
        cbind(names_5, meanSurv5_d, medianSurv5_d)
      ...
      survivalProbs_p5_tr <-
        predict(Mod5, sel_clin_tr2, type = "prob_times")
      # Test
      Survival_Probs_event5 <-
        predict(Mod5, sel_clin_te2, type = "prob_event")
      survivalProbs_p5 <-
        predict(Mod5, sel_clin_te2, type = "prob_times")
      # create a data frame combining predicted mean, median, survival
      # probability and actual time and event
      surv_res5 <- cbind(
        meanSurv5,
      ...
      # Concordance Index
      c_index_5_tr <- round(
        SurvMetrics::Cindex(surv_obj_5_tr,
          predicted = medianSurv5_tr
        ),
        2
      )
      ...
      # Concordance Index
      c_index_5 <- round(
        SurvMetrics::Cindex(surv_obj_5,
          predicted = medianSurv5
        ),
        2
      )
      # Combine evaluation parameters to get Matrix
