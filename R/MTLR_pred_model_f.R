...
#' MTLR_pred_model_f(train_clin_data = Train_Clin, test_clin_data =
...
#' Usage: MTLR_pred_model_f(train_clin_data, test_clin_data, Model_type,
...
MTLR_pred_model_f <- function(train_clin_data, test_clin_data, Model_type,
  ...
  if(Model_type == 1){
    ...
    surv_res1 <- cbind(meanSurv1, medianSurv1, Survival_prob_event1 ,
    ...
    # calculate IBS (Integrated Brier Score for test data
    IBS_1 <- round(IBS(surv_obj1, sp_matrix = survivalProbs_t_mat1_t2,
  ...
  ########################## Model with only PI score #######################
    ...
    #combine clinical and feature data
    sel_clin_tr1  <- cbind(tr_clin1["OS"], tr_clin1["OS_month"],
                           tr_data2["PI"])
    ...
    # calculate IBS (Integrated Brier Score for test data
    IBS1_2 <- round(IBS(surv_obj_2, sp_matrix = survivalProbs_t_mat1_t2_2,
    ...
    ## create a data frame combining predicted mean, median, survival
    # probability and actual time and event
    surv_res3 <- cbind(meanSurv3, medianSurv3, Survival_Probs_event3,
                       sel_clin_te2$OS_month, sel_clin_te2$OS)
    ...
    # calculate IBS (Integrated Brier Score for test data
    IBS1_3 <- round(IBS(surv_obj_3, sp_matrix = survivalProbs_t_mat1_t2_3,
    ...
    # create a data frame combining predicted mean, median, survival
    # probability and actual time and event
    surv_res4 <- cbind(meanSurv4, medianSurv4, Survival_prob_event4,
                       sel_clin_te2$OS_month, sel_clin_te2$OS )

    #add column names and row names
    colnames(surv_res4) <- c("Mean_Surv", "Median_surv", "Survival_Prob",
                             "Actual_OS_time", "OS_event")
    ...
    # calculate IBS (Integrated Brier Score) for test data
    IBS1_4 <- round(IBS(surv_obj_4, sp_matrix = survivalProbs_t_mat1_t2_4,
                        survivalProbs_p4$time[-1]),3)

    #Concordance Index
    c_index_4 <- round(SurvMetrics::Cindex(surv_obj_4,
    ...
    #add row and column names
    colnames(surv_res5) <- c("Mean", "Median_surv", "Survival_Prob_event",
                             "Actual_OS_time", "Event")
    ...
    # calculate IBS (Integrated Brier Score for test data
    IBS1_5<- round(IBS(surv_obj_5, sp_matrix = survivalProbs_t_mat1_t2_5,
  ...
  return(list(survCurves_data = survCurves_df,
